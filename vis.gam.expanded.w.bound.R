#this file allows you to use a polygon to clip the gam surface produced by vis.gam to be within the polygon.  It uses the expanded
#version of vis.gam that allows control of the legend and color map.  I developed it for the US NOAA report, and it currently has 
#commands from that workspace.

library(maps)
library(mapdata)


#code for function development
bound <- map(database = "usa", region = "main")

#the basic call
vis.gam.expanded(M.l.f.0,view = c("Longitude","Latitude"), plot.type = "contour", too.far = .05, main = "Fishing Gear", color = "rg", legend.loc = "bottomright", Bounded = T, Poly = bound)
map("state", add = T, bg = "white")
points(DataFull$Longitude,DataFull$Latitude, cex = .25, pch = 19)
index <- order(DataFull$Fishing_Items)[round(.95*dim(DataFull)[1]):dim(DataFull)[1]]
points(DataFull$Longitude[index],DataFull$Latitude[index], pch = 21, cex = .25, lwd = .25, bg = "white")

#so the above worked, but if you want a bound with a buffer, you need to do a couple extra steps
library(maptools)
library(raster)
IDs <- sapply(strsplit(bound$names, ":"), function(x) x[1])
bound.sp <- map2SpatialPolygons(bound, IDs=IDs, proj4string=CRS("+proj=longlat +datum=WGS84"))

#add buffer, note it is likely in meters if the locations are in lat/lon based on the help, but I am not so sure
bound.sp.buffered <- buffer(bound.sp, width=.5)
plot(bound.sp.buffered)

#now you need this little function to the coordinates back out, hard b/c is s4 w slots
getEdges <- function(x) {
...
#now we just want the coordinates
bound.buffered <- as.data.frame(getEdges(bound.sp.buffered))

#the basic call again, but with a buffer
vis.gam.expanded(M.l.f.0,view = c("Longitude","Latitude"), plot.type = "contour", too.far = .05, main = "Fishing Gear", color = "rg", legend.loc = "bottomright", Bounded = T, Poly = bound.buffered)
map("state", add = T, bg = "white")
points(DataFull$Longitude,DataFull$Latitude, cex = .25, pch = 19)
index <- order(DataFull$Fishing_Items)[round(.95*dim(DataFull)[1]):dim(DataFull)[1]]
points(DataFull$Longitude[index],DataFull$Latitude[index], pch = 21, cex = .25, lwd = .25, bg = "white")
...
#the basic call again, but with a buffer, and finer grid 
vis.gam.expanded(M.l.f.0,view = c("Longitude","Latitude"), plot.type = "contour", n.grid = 60, too.far = .05, main = "Fishing Gear", color = "rg", legend.loc = "bottomright", Bounded = T, Poly = bound.buffered)
map("state", add = T, bg = "white")
points(DataFull$Longitude,DataFull$Latitude, cex = .25, pch = 19)
index <- order(DataFull$Fishing_Items)[round(.95*dim(DataFull)[1]):dim(DataFull)[1]]
points(DataFull$Longitude[index],DataFull$Latitude[index], pch = 21, cex = .25, lwd = .25, bg = "white")
...
#note that you can also reverse the heat colors using heat.r as color.  This code also returns the legend values so you can make a legend outside the vis.gam code if you want.
